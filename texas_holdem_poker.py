    ...
    class GamePage(Frame):
        def __init__(self, parent, controller):
            ...
            dealer_pic = ImageTk.PhotoImage(
                Image.open("cards\dealer.png").resize((429, 284), Image.ANTIALIAS))
            self.dealer.image = dealer_pic
            self.dealer.configure(image=dealer_pic)
            
            frame_anal = Frame(canvas, bg='green', bd=5)
            frame_anal.place(relx=0, rely=1, relwidth=0.3, relheight=0.3, anchor='sw')
            name_frameanal = Frame(frame_anal, bg="white", bd=5)
            name_frameanal.place(relx=0.5, rely=0.5, relwidth=0.99, relheight=0.99, anchor="c")
